...
class FTPHandler(NetworkProtocolHandler):
    ...
    def execute_update(self, settings: NetworkSettings):
        ...
        # Verify connection and SSL certificate in case of FTPS
        if self.is_secure:
            # Check if we're using an IP address
            is_ip_address, error_message = self.validate_server.validate_server(server_url)
            
            if is_ip_address:
                self.logger.info(f"Using IP address for FTPS: {server_url}. Disabling hostname verification.")
                
                # For IP addresses, we need to modify the verification process
                # This will be handled in the NetworkVerifier class
                is_valid, error_message = self.verifier.verify_ftps_certificate_ip(final_url)
            else:
                is_valid, error_message = self.verifier.verify_ftps_certificate(final_url)
            ...
            if is_ip_address:
                is_valid, error_message = self.verifier.verify_ftps_connection_ip(final_url, settings.username, settings.password)
            else:
                is_valid, error_message = self.verifier.verify_ftps_connection(final_url, settings.username, settings.password)
            ...
            is_valid, error_message = self.verifier.verify_ftp_connection(final_url, settings.username, settings.password)
            if not is_valid:
                return False, error_message
